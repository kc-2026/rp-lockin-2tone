...
### Two lasers, not one — established 2026-08-25

Everything written before that date, here and elsewhere, describes a single
sweeping laser. That was incomplete.

- **The fine sweeper** covers a wavelength span in approximately one second,
  5000 points. Its trigger output goes to IN2, and its log supplies the
  wavelength axis. This is the one every older passage means by "the laser".
- **The stepper** sits at **11 discrete wavelengths, one per sweep**. It never
  sweeps in real time, so it has no trigger train and no log to index — it is
  set, allowed to settle, and read with `:WAVelength?`.

The deliverable across the eleven is an **11 x 5000 map** of response against
both wavelengths. `SweepSeries` in `pipeline.py` holds the set.
...
Both are **Santec** instruments (a TSL-770 and a TSL-775), and each can report
its own wavelength over a serial link. **Serial control of the stepper is
parked** at Kevin's request; `set_wavelength_m()` exists and nothing calls it.
...
**That the log holds one point per trigger pulse WAS an assumption, and it no
longer matters.** No Santec manual states it, and the mapping used to rest on
it — but only while the time step came from the trigger *interval*.
`pipeline.reduce_sweep` takes the step from the trigger train's **span** over
(N − 1) logged points, which never counts pulses. **Q26 is dead**; see
`11-pipeline.md`.

**Q24 is not.** The step arithmetic assumes the trigger is periodic in TIME, not
in wavelength, and the two manuals define `:TRIGger:OUTPut:SETTing` with
opposite encodings. Read it back on the bench; never hardcode a literal.
...
| IN2 | **Sweeping** laser's trigger train — the acquisition trigger source and the t = 0 anchor. **Must be on HV**: the trigger is 3.3 V, and the ±1 V range clips it into a flat line that reads as "the laser is not triggering". Gain and coupling are per channel — see `setup_channel()` |
