...
| Q18 | **Which trigger-output mode is the Santec set to — one pulse per sweep, or one per wavelength step?** Decides whether IN2 carries a single alignment edge or a pulse train. A train is not a problem in itself — each pulse would carry a known wavelength — but it brings back the missed-edge question at high decimation, and hence the memory-region question. Raised 2026-08-14. |
| Q19 | **How do the laser's timebase and the Red Pitaya's relate?** The serial report and the capture are on different clocks. The trigger fixes a common origin, but a clock-rate mismatch accumulates: 100 ppm over a 1 s sweep is 100 µs of drift, and whether that matters depends on how fast the wavelength moves. Needs the sweep rate and the tolerable wavelength error. |
| Q20 | **What exactly does the Santec return over serial — wavelength against absolute time, or against a step index?** If it is indexed rather than timestamped, the mapping still needs the sweep timing, which changes how much the trigger has to do. Also needed: the serial command set, the port settings, and whether the data is streamed live or dumped after the sweep. This is new code on the control PC (a small Santec driver, probably `pyserial`) and is not yet in scope anywhere. |
