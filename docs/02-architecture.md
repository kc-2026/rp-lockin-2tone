...
This is why the capture planner recommends decimation 2 rather than 1.
...
All were live, all produced believable wrong answers, all now have tests.
...
A fourth, in the emulator rather than the library: clipping protection rescaled
the synthesised waveform but not the recorded ground truth, so a loopback test
would have reported a phantom 2× amplitude error. Caught by
